...
if(! defined('WATERMARK_IMAGE_ENABLED'))define( 'WATERMARK_IMAGE_ENABLED', true);					// the watermark image
...
class timthumb {
    ...
    protected function processImageAndWriteToCache($localImage){
        ...
        //$watermark_image_details = $this->param('wmi', 0);
        $watermark_position = $this->param('wmp', 'r');
        ...
        // watermark image
        if (WATERMARK_IMAGE_ENABLED && preg_match('/^image\/(?:jpg|jpeg)$/i', $mimeType)) {
            //$watermark_position = 'r';
            $watermark_opacity = 40;

            $watermark_image = null;
            if ($new_width > 200 && $new_height > 160) {
                $watermark_image = dirname($this->localImage). DIRECTORY_SEPARATOR . 'marcadagua.png';
                $watermark_margin = 15;
            }
            elseif ($new_width > 70 && $new_height > 70) {
                $watermark_image = dirname($this->localImage). DIRECTORY_SEPARATOR . 'marcadagua-small.png';
                $watermark_margin = 5;
            }
            //if ($watermark_image_details && WATERMARK_IMAGE_ENABLED) {
            if (!is_null($watermark_image) && file_exists($watermark_image)) {
                ...
                if(!file_exists($watermark_image)) die('Can not found watermark image.'.$watermark_image);
                $watermark_image = imagecreatefrompng( $watermark_image );
                $watermark_width = imagesx( $watermark_image );
                $watermark_height = imagesy( $watermark_image );

                /*
                $maxwm_width = floor($new_width / 2);
                $maxwm_height = floor($new_height / 3);

                if ($watermark_height > $maxwm_height) {
                    $watermark_height = $maxwm_height;
                    $watermark_width = floor(($new_width * $watermark_height) / $new_height);
                }
                if ($watermark_width > $maxwm_width) {
                    $watermark_width = $maxwm_width;
                    $watermark_height = floor(($new_height * $watermark_width) / $new_width);
                }
                */
                ...
                if(strtolower($watermark_position) == 'c'){
                    ...
                    $watermark_positionx = $new_width - ($watermark_width + $watermark_margin);
                    $watermark_positiony = $new_height - ($watermark_height + $watermark_margin);
                }
                /*
                if(strtolower($watermark_position) == 'c'){
                    $watermark_positionx = $new_width/2 - $watermark_width/2;
